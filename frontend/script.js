...
function initHomeCharts() {
  ...
  const chartOptions = {
    ...
    plugins: {
      legend: {
        position: 'bottom',
        labels: {
          color: '#8A8A90', // var(--pnb-text-muted)
          usePointStyle: true,
          padding: 20
        }
      }
  ...
  const pnbColors = {
    ...
    danger: '#EF4444',
    muted: '#8A8A90',
    bg: '#1C1C1F' // var(--pnb-surface-2) equivalent for grid tests
  };
    ...
    CbomCharts.cipherUsage = new Chart(ctxCipher, {
      ...
      options: {
        ...
        scales: {
          ...
          y: {
            grid: { display: false, drawBorder: false },
            ticks: { color: pnbColors.muted, font: { family: 'IBM Plex Mono' } }
          }
        }
      }
    });
